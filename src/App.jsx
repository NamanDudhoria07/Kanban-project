...
function App() {
  ...
  return (
    <div style={{
      ...
      fontFamily: '"Inter", sans-serif', transition: 'background-image 0.5s ease',
      boxSizing: 'border-box', overflowX: 'hidden'
    }}>
      <style>{`
        .task-card { 
          touch-action: none; /* CRITICAL: Enables dragging on mobile */
          cursor: grab;
          user-select: none;
      ...
      <div style={{ width: '100%', maxWidth: '1400px', position: 'relative' }}>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
            <span style={{ fontSize: '0.8rem', fontWeight: '900', color: isDarkMode ? '#cbd5e1' : '#475569', letterSpacing: '0.08em' }}>BOARD PROGRESS</span>
          ...
          <div style={{ width: '100%', height: '10px', backgroundColor: 'rgba(0,0,0,0.1)', borderRadius: '12px', overflow: 'hidden' }}>
            <div className="progress-bar" style={{ 
              ...
              boxShadow: progress > 0 ? '0 0 15px rgba(37, 99, 235, 0.4)' : 'none'
            }}></div>
          </div>
        </div>
        ...
        <header style={{ textAlign: 'center', marginBottom: '40px', paddingTop: '60px' }}>
          <h1 style={{ fontSize: 'clamp(2.2rem, 8vw, 3.8rem)', fontWeight: '900', color: isDarkMode ? '#f8fafc' : '#1f2937', marginBottom: '20px' }}>Kanban Board</h1>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '15px' }}>
            ...
            <button onClick={() => setShowModal(true)} style={{ backgroundColor: '#2563eb', color: 'white', padding: '14px 36px', borderRadius: '16px', border: 'none', fontWeight: 'bold', cursor: 'pointer' }}>+ Add New Task</button>
          </div>
        </header>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '30px', width: '100%' }}>
          {['todo', 'inProgress', 'done'].map((col) => (
            <div key={col} 
              ...
              style={{ backgroundColor: isDarkMode ? 'rgba(15, 23, 42, 0.82)' : 'rgba(255, 255, 255, 0.45)', backdropFilter: 'blur(22px)', padding: '25px', borderRadius: '32px', minHeight: '550px', display: 'flex', flexDirection: 'column' }}
            >
                ...
                {tasks[col]
                  ...
                  >
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
                      ...
                      <button onClick={() => deleteTask(col, task.id)} style={{ color: '#ef4444', border: 'none', background: 'none', cursor: 'pointer', fontWeight: 'bold' }}>×</button>
                    </div>
      ...
      {showModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', backgroundColor: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(10px)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 100 }}>
          <div style={{ backgroundColor: isDarkMode ? '#1e293b' : 'white', padding: '35px', borderRadius: '28px', width: '90%', maxWidth: '420px' }}>
            <h2 style={{ marginBottom: '25px', color: isDarkMode ? '#f1f5f9' : '#111827' }}>New Task</h2>
            <form onSubmit={handleSaveTask}>
              <input type="text" placeholder="Title" required style={{ width: '100%', padding: '14px', marginBottom: '15px', borderRadius: '12px' }} onChange={(e) => setNewTask({...newTask, title: e.target.value})} />
              <textarea placeholder="Description" style={{ width: '100%', padding: '14px', marginBottom: '15px', borderRadius: '12px', height: '100px' }} onChange={(e) => setNewTask({...newTask, description: e.target.value})} />
              <select style={{ width: '100%', padding: '14px', marginBottom: '25px', borderRadius: '12px' }} onChange={(e) => setNewTask({...newTask, priority: e.target.value})}>
