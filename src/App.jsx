...
function App() {
  ...
  const [touchTaskId, setTouchTaskId] = useState(null); // Tracking for mobile
  ...
  const onDrop = (e, destCol) => {
    ...
    moveTaskLogic(taskId, sourceCol, destCol);
  };

  // --- SHARED MOVEMENT LOGIC ---
  const moveTaskLogic = (taskId, sourceCol, destCol) => {
  ...
  return (
    <div style={{
      ...
      padding: 'clamp(20px, 5vw, 60px) clamp(10px, 3vw, 20px)', fontFamily: '"Inter", sans-serif',
      transition: 'background-image 0.5s ease-in-out', boxSizing: 'border-box', overflowX: 'hidden'
    }}>
      <style>{`
        .kanban-column { transition: all 0.3s ease !important; }
        .task-card { 
            transition: transform 0.2s ease, background-color 0.2s ease !important; 
            display: flex; align-items: flex-start; gap: 12px;
            touch-action: manipulation;
        }
        .task-card.selected { border: 2px solid #2563eb !important; background-color: rgba(37, 99, 235, 0.1) !important; }
        .drag-handle { cursor: grab; color: ${isDarkMode ? '#475569' : '#cbd5e1'}; font-size: 1.2rem; padding-top: 2px; user-select: none; }
      ...
      <div style={{ width: '100%', maxWidth: '1400px', position: 'relative' }}>
        ...
        <header style={{ textAlign: 'center', marginBottom: '40px', paddingTop: '60px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '15px' }}>
            ...
            <button onClick={() => { setIsEditing(null); setNewTask({ title: '', description: '', priority: 'low' }); setShowModal(true); }} style={{ backgroundColor: '#2563eb', color: 'white', padding: '14px 36px', borderRadius: '16px', border: 'none', fontWeight: 'bold' }}>+ Add New Task</button>
          </div>
        </header>
          ...
          <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', backgroundColor: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(10px)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 100 }}>
            <div style={{ backgroundColor: isDarkMode ? '#1e293b' : 'white', padding: '35px', borderRadius: '28px', width: '90%', maxWidth: '420px' }}>
              ...
              <form onSubmit={handleSaveTask}>
                <input type="text" placeholder="Title" required value={newTask.title} onChange={(e) => setNewTask({...newTask, title: e.target.value})} style={{ width: '100%', padding: '14px', marginBottom: '15px', borderRadius: '12px', border: '1px solid #ccc', boxSizing: 'border-box' }} />
                <textarea placeholder="Description" value={newTask.description} onChange={(e) => setNewTask({...newTask, description: e.target.value})} style={{ width: '100%', padding: '14px', marginBottom: '15px', borderRadius: '12px', border: '1px solid #ccc', height: '100px', boxSizing: 'border-box' }} />
                <select value={newTask.priority} onChange={(e) => setNewTask({...newTask, priority: e.target.value})} style={{ width: '100%', padding: '14px', marginBottom: '25px', borderRadius: '12px', border: '1px solid #ccc', boxSizing: 'border-box' }}>
                  <option value="high">High Priority</option>
                  <option value="medium">Medium Priority</option>
                  <option value="low">Low Priority</option>
                </select>
                <div style={{ display: 'flex', gap: '12px' }}>
                  <button type="submit" style={{ flex: 1, backgroundColor: '#2563eb', color: 'white', padding: '14px', borderRadius: '12px', border: 'none' }}>Save</button>
                  <button type="button" onClick={() => setShowModal(false)} style={{ flex: 1, backgroundColor: '#ccc', padding: '14px', borderRadius: '12px', border: 'none' }}>Cancel</button>
                </div>
              </form>
            </div>
          </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '30px', width: '100%' }}>
          {['todo', 'inProgress', 'done'].map((col) => (
            <div 
              ...
              onClick={() => touchTaskId && moveTaskLogic(touchTaskId.id, touchTaskId.col, col)}
              className="kanban-column"
              style={{ 
                backgroundColor: isDarkMode ? 'rgba(15, 23, 42, 0.82)' : 'rgba(255, 255, 255, 0.45)', 
                backdropFilter: 'blur(22px)', padding: '25px', borderRadius: '32px', minHeight: '550px', 
                border: touchTaskId ? '2px dashed #2563eb' : '1px solid rgba(255,255,255,0.1)'
              }}
            >
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                {tasks[col]
                  ...
                  <div 
                    key={task.id} 
                    className={`task-card ${touchTaskId?.id === task.id ? 'selected' : ''}`}
                    draggable 
                    onDragStart={(e) => onDragStart(e, task.id, col)}
                    style={{ 
                      backgroundColor: isDarkMode ? '#1e293b' : 'white', padding: '22px', borderRadius: '20px', 
                      boxShadow: '0 4px 12px rgba(0,0,0,0.05)', position: 'relative'
                    }}
                  >
                    {/* 6-DOT HANDLE: Tap on mobile to select */}
                    <div className="drag-handle" onClick={(e) => { e.stopPropagation(); setTouchTaskId({id: task.id, col}); }}>⋮⋮</div>
                    ...
                    <div style={{ flex: 1 }}>
                        ...
                        <h3 style={{ margin: '0 0 6px 0', fontSize: '1.1rem', color: isDarkMode ? '#f1f5f9' : '#1f2937' }}>{task.title}</h3>
